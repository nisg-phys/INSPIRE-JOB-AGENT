"""Client for INSPIRE-HEP's literature API.

Mirrors worker/worker/inspire_literature_client.py. Kept as a separate
copy rather than a shared dependency since backend and worker are
independently deployable packages - the backend only needs this for the
on-demand enrichment fallback in institution_papers.fetch_and_store
(see its docstring for why that exists).
"""
